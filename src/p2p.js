...
// Тестове WebSocket-з’єднання для дебагу
if (isBrowser) {
    const wsUrl = isLocalhost ? 'ws://localhost:3000/ws' : 'wss://libp2p.onrender.com/ws';
    const ws = new WebSocket(wsUrl);
    ws.onopen = () => debugLogger('INFO: WebSocket connection established to ' + wsUrl);
    ws.onerror = (err) => debugLogger('ERROR: WebSocket connection failed to ' + wsUrl + ': %o', err);
    ws.onmessage = (msg) => debugLogger('INFO: WebSocket message received: %o', msg.data);
}
...
function saveRedirectsCacheToLocalStorage() {
    try {
        ...
        debugLogger('ERROR: Failed to save redirectsCache to localStorage: %o', err);
    }
}
...
function loadRedirectsCacheFromLocalStorage() {
    try {
        ...
        debugLogger('ERROR: Failed to load redirectsCache from localStorage: %o', err);
    }
}
...
function clearOldRedirectData() {
    try {
        ...
        debugLogger('ERROR: Failed to clear old redirect data: %o', err);
    }
}
...
async function publishNodeAddress() {
    ...
    const multiaddrs = node.getMultiaddrs().map(ma => ma.toString()).filter(addr => 
    ...
    try {
        ...
        debugLogger('INFO: Published node address to DHT: ' + nodeKey + ' with multiaddrs: %o', multiaddrs);
    } catch (err) {
        ...
        for await (const provider of node.services.dht.findProviders(uint8ArrayFromString(prefix), DHT_GET_OPTIONS)) {
            ...
            try {
                ...
                if (nodeData.multiaddrs && Array.isArray(nodeData.multiaddrs)) {
                    ...
                    debugLogger('INFO: Discovered node: ' + key + ' with addresses: %o', nodeData.multiaddrs);
                    if (peerId === ownPeerId) {
                        for (const addr of nodeData.multiaddrs) {
                            try {
                                await node.dial(multiaddr(addr), { timeout: 5000 });
                                debugLogger('INFO: Successfully dialed own address: ' + addr);
                            } catch (err) {
                                debugLogger('ERROR: Failed to dial own address ' + addr + ': %o', err);
                            }
                        }
                    }
                }
            } catch (err) {
                debugLogger('ERROR: Failed to fetch node data for ' + key + ': %o', err);
            }
        }
    } catch (err) {
...
async function requestStaticFile(filePath) {
    ...
    try {
        ...
        if (contentBytes) {
            ...
            debugLogger('INFO: Fetched static file ' + filePath + ' from DHT');
            return content;
        }
    } catch (err) {
        debugLogger('ERROR: Failed to fetch static file ' + filePath + ': %o', err);
    }
    ...
    try {
        ...
        if (!providers.length) {
            debugLogger('WARN: No providers found for file ' + filePath);
            return null;
        }
    } catch (err) {
        debugLogger('ERROR: Failed to find providers for file ' + filePath + ': %o', err);
        return null;
    }
    ...
    for (const provider of providers) {
        try {
            ...
            localStorage.setItem('site-file:' + filePath, content);
            debugLogger('INFO: Fetched static file ' + filePath + ' from peer ' + provider.id.toString());
            return content;
        } catch (err) {
            debugLogger('ERROR: Failed to fetch static file ' + filePath + ' from peer ' + provider.id.toString() + ': %o', err);
        }
    }
...
async function loadSiteFromP2P() {
    ...
    if (!localStorage.getItem('site-file:' + criticalFile)) {
        ...
        if (!content) {
            debugLogger('ERROR: Failed to load critical file ' + criticalFile);
...
async function loadNonCriticalFiles() {
    ...
    for (const file of nonCriticalFiles) {
        if (!localStorage.getItem('site-file:' + file)) {
            ...
            if (content) {
                debugLogger('INFO: Fetched background file ' + file);
            }
        }
    }
}
    ...
    for (const file of staticFiles) {
        ...
        if (content) {
            try {
                ...
                debugLogger('INFO: Published static file to DHT: ' + file.key);
            } catch (err) {
                debugLogger('ERROR: Failed to publish static file ' + file.key + ': %o', err);
            }
        }
    }
        ...
        for (const file of allFiles) {
            ...
            if (content) {
                node.services.dht.put(
                    ...
                    debugLogger('ERROR: Failed to republish static file ' + file.key + ': %o', err);
                });
            }
        }
    }, 5 * 60 * 1000);
...
async function startNodeInternal() {
    ...
    try {
        ...
        // Динамічне імпортування mDNS для локального тестування
        let mdnsDiscovery;
        if (enableMDNS) {
            try {
                const mdnsModule = await import('@libp2p/mdns');
                mdnsDiscovery = mdnsModule.mdns;
                debugLogger('INFO: mDNS dynamically loaded for local testing');
            } catch (err) {
                debugLogger('ERROR: Failed to load mDNS module: %o', err);
                updateP2PStatus('mDNS load failed', true);
            }
        }
        ...
        const wsFilter = (ma) => {
            ...
            if (!isLocalhost && addrStr.includes('ws://')) {
                ...
                debugLogger('INFO: Replaced ws:// with wss://: ' + addrStr);
            }

            if (addrStr.includes('wss//')) {
                addrStr = addrStr.replace('wss//', 'wss/');
                debugLogger('INFO: Fixed wss// to wss/: ' + addrStr);
            }

            if (addrStr.includes('/ip4/') && !addrStr.includes('/tcp/')) {
                addrStr = addrStr.replace('/ip4/', '/ip4/tcp/');
                debugLogger('INFO: Added /tcp to IP4 multiaddr: ' + addrStr);
            }

            debugLogger('INFO: Final WebSocket/TCP multiaddr: ' + addrStr);
        ...
        debugLogger('INFO: Libp2p node created with ID: ' + node.peerId.toString());
        ...
        node.handle(STATIC_FILES_PROTOCOL, async ({ stream, connection }) => {
            try {
                ...
                if (content) {
                    ...
                    debugLogger('INFO: Served static file ' + filePath + ' to peer ' + connection.remotePeer.toString());
                } else {
                    await stream.close();
                    debugLogger('WARN: File ' + filePath + ' not found for peer ' + connection.remotePeer.toString());
                }
            } catch (err) {
        ...
        node.addEventListener('peer:discovery', (evt) => {
            ...
            debugLogger('INFO: Discovered peer: ' + peerId + ' with multiaddrs: %o', multiaddrs);
            updateP2PStatus('Discovered peer: ' + peerId.substring(0, 10) + '...');
        });
        node.addEventListener('peer:connect', (evt) => {
            const peerId = evt.detail.toString();
            debugLogger('INFO: Connected to peer: ' + peerId);
        ...
        node.addEventListener('peer:disconnect', (evt) => {
            const peerId = evt.detail.toString();
            debugLogger('INFO: Disconnected from peer: ' + peerId);
        ...
        debugLogger('INFO: Libp2p node started with ID: ' + node.peerId.toString());
        ...
        const dialPromises = bootstrapMultiaddrs.map(async (addr) => {
            try {
                ...
                debugLogger('INFO: Attempting to dial bootstrap node: ' + addr);
                await node.dial(ma, { timeout: 5000 });
                debugLogger('INFO: Successfully dialed bootstrap node: ' + addr);
                successfulConnections++;
            } catch (err) {
                debugLogger('ERROR: Failed to dial bootstrap node ' + addr + ': %o', err);
            }
        });
        ...
        if (node.services?.pubsub) {
            ...
            debugLogger('INFO: Subscribed to PubSub topic: ' + topic);
        ...
        if (successfulConnections === 0) {
            ...
            if (dhtAddrs.length > 0) {
                const dhtDialPromises = dhtAddrs.map(async (addr) => {
                    try {
                        const ma = multiaddr(addr);
                        debugLogger('INFO: Attempting to dial DHT node: ' + addr);
                        await node.dial(ma, { timeout: 5000 });
                        debugLogger('INFO: Successfully dialed DHT node: ' + addr);
                        successfulConnections++;
                    } catch (err) {
                        debugLogger('ERROR: Failed to dial DHT node ' + addr + ': %o', err);
                    }
                });
        ...
        if (successfulConnections === 0) {
            ...
            debugLogger('INFO: Connected to ' + successfulConnections + ' node(s)');
        ...
        setTimeout(() => {
            ...
            setInterval(async () => {
                ...
                for (const addr of newAddrs) {
                    try {
                        const ma = multiaddr(addr);
                        debugLogger('INFO: Attempting to dial discovered node: ' + addr);
                        await node.dial(ma, { timeout: 5000 });
                        debugLogger('INFO: Successfully dialed discovered node: ' + addr);
                    } catch (err) {
                        debugLogger('ERROR: Failed to dial discovered node ' + addr + ': %o', err);
                    }
                }
            }, 5 * 60 * 1000);
...
async function handlePubsubMessage(evt) {
    debugLogger('INFO: Received PubSub message for topic: ' + evt.detail.topic);
...
async function createRedirect(url, description = '') {
    ...
    if (!url || typeof url !== 'string' || url.length < 5) {
        debugLogger('ERROR: Invalid URL provided: ' + url);
        throw new Error('Invalid URL provided');
    }

    debugLogger('INFO: Checking network connectivity');
    const connectedPeers = node?.getPeers ? node.getPeers() : [];
    debugLogger('INFO: Connected peers: %o', connectedPeers.map(p => p.toString()));
    const isIsolated = !node || connectedPeers.length === 0 || !node.services?.dht;
    if (isIsolated) {
    ...
    if (!success) {
        debugLogger('ERROR: Failed to generate unique shortCode after ' + MAX_SHORTCODE_GENERATION_ATTEMPTS + ' attempts');
    ...
    debugLogger('INFO: ShortCode generated: ' + shortCode);
    ...
    try {
        debugLogger('INFO: Saving redirect to DHT: ' + key);
        if (!isIsolated && node.services?.dht) {
            await node.services.dht.put(
                uint8ArrayFromString(key),
    ...
    debugLogger('INFO: Publishing create message for: ' + shortCode);
    ...
    debugLogger('INFO: Caching redirect locally: ' + shortCode);
    redirectsCache.set(shortCode, redirect);
    saveRedirectsCacheToLocalStorage();
...
async function getRedirect(shortCode) {
    debugLogger('INFO: getRedirect called with: ' + shortCode);
    if (!shortCode) {
...
async function updateRedirect(shortCode, newUrl, newDescription, redirectPassword) {
    ...
    if (!newUrl || typeof newUrl !== 'string' || newUrl.length < 5) {
        debugLogger('ERROR: updateRedirect: Invalid new URL: ' + newUrl);
    ...
    debugLogger('INFO: Verifying password for: ' + shortCode);
    ...
    const key = KEY_PREFIX + shortCode;
    try {
        debugLogger('INFO: Saving updated redirect to DHT: ' + key);
        if (!isIsolated && node.services?.dht) {
            await node.services.dht.put(
                uint8ArrayFromString(key),
    ...
    debugLogger('INFO: Publishing update message for: ' + shortCode);
    ...
    debugLogger('INFO: Updating local cache for: ' + shortCode);
    redirectsCache.set(shortCode, updatedRedirect);
    saveRedirectsCacheToLocalStorage();
...
async function deleteRedirect(shortCode, redirectPassword) {
    ...
    debugLogger('INFO: Verifying password for deletion: ' + shortCode);
    const isValidPassword = await verifyRedirectPassword(redirectPassword, stored.passwordHash);
    if (!isValidPassword) {
    ...
    const isIsolated = !node || !node.getPeers || node.getPeers().length === 0 || !node.services?.dht;
    debugLogger('INFO: Publishing delete message for: ' + shortCode);
...
function getLocalRedirects(searchQuery = '') {
    debugLogger('INFO: getLocalRedirects called with query: ' + searchQuery);
...
function generatePassword(length = 12) {
    debugLogger('INFO: generatePassword called with length: ' + length);
    ...
    debugLogger('INFO: Generated password: ' + password);
    return password;
}

function generateSalt(length = 16) {
    debugLogger('INFO: generateSalt called with length: ' + length);
    if (typeof crypto !== 'undefined' && crypto.getRandomValues) {
        const values = new Uint8Array(length);
        crypto.getRandomValues(values);
        const salt = Array.from(values, byte => byte.toString(16).padStart(2, '0')).join('');
        debugLogger('INFO: Generated salt using crypto: ' + salt);
        return salt;
    } else {
        ...
        debugLogger('INFO: Generated salt using Math.random: ' + salt);
        return salt;
    }
}
...
async function hashPassword(password, salt = null) {
    ...
    if (isCryptoAvailable) {
        ...
        try {
            ...
            debugLogger('INFO: Password hashed: ' + result);
            return result;
        } catch (err) {
        ...
        const result = currentSalt + ':' + hashHex;
        debugLogger('INFO: Password hashed with crypto-browserify: ' + result);
        return result;
    }
}
...
async function verifyRedirectPassword(providedPassword, storedSaltAndHash) {
    ...
    try {
        ...
        debugLogger('INFO: Password verification result: ' + isValid);
        return isValid;
    } catch (error) {
...
async function generateShortCode(inputString) {
    debugLogger('INFO: generateShortCode called with: ' + inputString);
    if (isCryptoAvailable) {
        ...
        try {
            ...
            debugLogger('INFO: Generated shortCode: ' + shortCode);
            return shortCode;
        } catch (err) {
        ...
        const hashHex = hash.digest('hex');
        const shortCode = hashHex.slice(0, 10);
        debugLogger('INFO: Generated shortCode with crypto-browserify: ' + shortCode);
        return shortCode;
    }
}

// Дебаг функція для перевірки статусу
